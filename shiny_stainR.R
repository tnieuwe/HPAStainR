...
ui <- fluidPage(
  ...
  sidebarLayout(  
    ...
    sidebarPanel("Provide a list of proteins (genes), of standard human nomenclature, and HPAStain.R will output the cell type they are most associated with based on patterns of staining in the Human Protein Atlas.",
                 ...
                 selectInput("cancer_analysis", "Incorporate cancer staining?", c("No", "Yes")),
...
server <- function(input, output){
  ...
  stainR <- function(gene_list, hpa_dat,
                     cancer_dat,
                     cancer_analysis = c("No", "Yes", "Only"), # weight_stain = F, weight_reliability = F,
                     tissue_level = T,
                     stringency = "normal",
                     scale_abundance= T,
    ...
    #Adding CANCER dat
    if (cancer_analysis == "Yes" | cancer_analysis == "Only") {
    ...
    if (scale_genes == T) {
      ...
      #CANCER
      if (cancer_analysis == "Yes") {
        cancer_gene_table  <- table(sub_cancer$Cancer,sub_cancer$Gene.name) > 0.5
