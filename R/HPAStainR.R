...
HPAStainR <- function(gene_list,
    ...
    if (cancer_analysis == "normal" | cancer_analysis == "both") {
        if (!is.data.frame(hpa_dat)) {
            stop(pste0("Are you sure hpa_dat is a dataframe? Download through", "HPADownloader if you're having issues"))
        }
    }
    
    if (cancer_analysis == "cancer" | cancer_analysis == "both") {
        if (!is.data.frame(cancer_dat)) {
            stop(paste("Are you sure cancer_dat is a dataframe? Download through", "HPADownloader if you're having issues"))
        }
    }
    ...
    ## Make gene list robust to incongruencies---------- test if comma separated or non comma separated
    ...
    hpa_dat <- hpa_dat %>% mutate(Tissue = gsub("[[:digit:]]+", "", Tissue), tissue_cell = paste0(toupper(str_trim(Tissue)), 
        " - ", Cell.type))
    ...
    ## Below code returns dataframe full of NAs in the case of no protein coding genes, this way you know where
    ## data is missing
    if (percent_coding == 0) {
        no_dat_matrix <- matrix(data = NA, nrow = length(all_cell_types), ncol = 7)
        rownames(no_dat_matrix) <- all_cell_types
        colnames(no_dat_matrix) <- c("High", "Medium", "Low", "Not detected", "staining_score", "num_genes", 
            "genes")
    ...
    # cell_type_dat_tiss_scale = cell_type_dat/tiss_scale Normalize based on how many times they are detected
    ...
    if (cancer_analysis == "both" | cancer_analysis == "Only") {
        ...
        cancer_count <- sub_cancer %>% group_by(Cancer) %>% summarise(High = sum(High), Medium = sum(Medium), 
            Low = sum(Low), `Not detected` = sum(Not.detected))
    ...
    ## Below is where we generate the final tibble----------------- 1. Make the data a tibble with rownames as
    ## cell_type
    cell_type_out <- as_tibble(cell_type_dat_per, rownames = "cell_type") %>% # 2. Round all numeric columns according to arguement
    mutate_if(is.numeric, round, round_to) %>% # 3. Select columns
    select(cell_type, High, Medium, Low, `Not detected`) %>% # 4. Generate the staining score
    mutate(staining_score = (High * 100) + (Medium * 50) + (Low * 25), num_genes = sum(gene_list %in% sub_dat$Gene.name)) %>% 
    ...
    cell_type_count <- as_tibble(cell_type_dat_mat, rownames = "cell_type") %>% rename(high_expression_count = "High", 
        medium_expression_count = "Medium", low_expression_count = "Low", not_detected_count = "Not detected")
    ...
    ## Tissue level determines if the analysis looks at cell types on a tissue level instead of a general
    ## level------
    
    ## This is ideal as cell types in different tissues, though similar, have different profiles.
    if (tissue_level == TRUE) {
        ...
        if (cancer_analysis == "both" | cancer_analysis == "Only") {
            ...
            cancer_staining_tf_df <- as.matrix.data.frame(table(cancer_staining_dat$Cancer, cancer_staining_dat$Gene.name) > 
                0, TRUE)
    ...
    ### CHI TEST HERE ------------- Insert chi square test here, first normal data, then cancer, then both Make
    ### sure to csv names filter cell type out to remove NAs
    
    
    ## Remove NAs and Testis as we don't use it
    ubi_test <- hpa_dat %>% mutate(stained = ifelse(Level != "Not detected", TRUE, FALSE), in_list = Gene.name %in% 
        gene_list) %>% filter(Tissue != "testis", !is.na(Cell.type), tissue_cell != "N/A - N/A")
    ...
    ## Cut the data down to rare genes found in less thant the 1st quartile quart_ind <- out[out <= quantile(out,
    ## .25)]
    ...
    not_2_levels <- quart_hpa %>% group_by(tissue_cell) %>% summarise(stain_mean = mean(stained), list_mean = mean(in_list)) %>% 
    ...
    if (nrow(quart_hpa) != 0) {
        chi_out <- quart_hpa %>% group_by(tissue_cell) %>% summarise(p_val = chisq.test(stained, in_list, simulate.p.value = TRUE)$p.value) %>% 
    ...
    if (cancer_analysis == "both" | cancer_analysis == "Only") {
        ...
        sub_canc <- sub_canc %>% mutate(stained = ifelse(Not.detected != 0, TRUE, FALSE), in_list = ifelse(Gene.name %in% 
            gene_list, TRUE, FALSE))
        
        ## Remove NAs and Testis as we don't use it
        ubi_test <- cancer_dat %>% mutate(stained = ifelse(Not.detected != 0, TRUE, FALSE), in_list = ifelse(Gene.name %in% 
            gene_list, TRUE, FALSE))
        ## Get a list of tested genes
        genes_tested <- table(ubi_test$Gene.name)
        ## Filter down to stained genes
        ...
        ## Not 2 levels is used to remove cell types that fail to have two levels in the gene list
        not_2_levels <- quart_canc %>% filter(!is.na(stained)) %>% group_by(Cancer) %>% summarise(stain_mean = mean(stained), 
            list_mean = mean(in_list)) %>% filter(stain_mean == 1 | stain_mean == 0 | list_mean == 1 | list_mean == 
            0)
        ...
        if (nrow(quart_canc) != 0) {
            chi_out <- quart_canc %>% group_by(Cancer) %>% summarise(p_val = chisq.test(stained, in_list, simulate.p.value = TRUE)$p.value) %>% 
    ...
    cell_type_out <- cell_type_out %>% mutate(p_val = format.pval(p_val, round_to, 0.005), p_val_adj = format.pval(p_val_adj, 
        round_to, 0.005))
    ...
    ## Change names-----------------
    if (csv_names == TRUE) {
        cell_type_out <- cell_type_out %>% select(cell_type, percent_high_expression = High, high_expression_count, 
            percent_medium_expression = Medium, medium_expression_count, percent_low_expression = Low, low_expression_count, 
            percent_not_detected = `Not detected`, not_detected_count, number_of_proteins = num_genes, tested_proteins = genes, 
            detected_proteins = stained_list, everything())
    }
    
    if (csv_names == FALSE) {
        cell_type_out <- cell_type_out %>% mutate(High = percent(High, acccuracy = 0.1), Medium = percent(Medium, 
            accuracy = 0.1), Low = percent(Low, accuracy = 0.1), `Not detected` = percent(`Not detected`, accuracy = 0.1)) %>% 
            select(`Cell Type` = cell_type, `Percent High Expression` = High, `High Expression Count` = high_expression_count, 
                `Percent Medium Expression` = Medium, `Medium Expression Count` = medium_expression_count, `Percent Low Expression` = Low, 
                `Low Expression Count` = low_expression_count, `Percent Not Detected` = `Not detected`, `Not Detected Count` = not_detected_count, 
                `Number of Proteins` = num_genes, `Staining Score` = staining_score, `Tested Proteins` = genes, 
                `Detected Proteins` = stained_list, `P-Value` = p_val, `P-Value Adjusted` = p_val_adj, everything())
    }
